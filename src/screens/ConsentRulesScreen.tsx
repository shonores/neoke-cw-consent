    ...
    <motion.div variants={variants} initial="initial" animate="animate" exit="exit" className="flex-1 flex flex-col bg-[var(--bg-ios)] min-h-screen">
      ...
      <main className="flex-1 px-5 pb-28 space-y-3">
        {loading ? (
          <>
            {[1, 2, 3].map(i => (
              <div key={i} className="animate-pulse bg-white rounded-[24px] h-24 w-full shadow-sm" />
            ))}
          </>
        ) : error ? (
              ...
              >
                <div className="px-4 py-4">
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${rule.enabled ? 'bg-green-100 text-green-700' : 'bg-[#f1f1f3] text-[#8e8e93]'}`}>
